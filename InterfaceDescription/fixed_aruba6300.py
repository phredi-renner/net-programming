    ...
    with open(outputfile, 'w') as output:
        ...
        for i in range(3, page.max_row+1, 1):
            ...
            if 'AP' in page.cell(row=i, column=1).value:
                output.write(f'interface Ethernet1/1/' + str(page.cell(row=i, column=3).value) + '\n')
                output.write(f' description '+ str(bldg) + '-' + str(idf) + '-' + str(page.cell(row=i, column=1).value) + '\n')
                output.write(' vlan access ' + str(d_vlan) + '\n')
                output.write(' spanning-tree bpdu-guard\n')
                output.write(' loop-protect\n')
                output.write(' loop-protect action tx-rx-disable\n')
            ...
            # if it's a camera then we have a different vlan
                ...
                output.write(f'interface Ethernet1/1/' + str(page.cell(row=i, column=3).value) + '\n')
                output.write(f' description '+ str(bldg) + '-' + str(idf) + '-' + str(page.cell(row=i, column=1).value) + '\n')
                output.write(' vlan access ' + str(s_vlan) + '\n')
                output.write(' spanning-tree bpdu-guard\n')
                output.write(' loop-protect\n')
                output.write(' loop-protect action tx-rx-disable\n')
                output.write(' no shutdown\n')
                ...
                output.write(f'interface Ethernet1/1/' + str(page.cell(row=i, column=3).value) + '\n')
                output.write(f' description '+ str(bldg) + '-' + str(idf) + '-' + str(page.cell(row=i, column=1).value) + '\n')
                output.write(' vlan access ' + str(sp_vlan) + '\n')
                output.write(' spanning-tree bpdu-guard\n')
                output.write(' loop-protect\n')
                output.write(' loop-protect action tx-rx-disable\n')
                output.write(' no shutdown\n')
            ...
            #    output.write(' switchport access vlan ' + str(d_vlan) + '\n')
                output.write(' vlan trunk native ' + str(d_vlan) + '\n')
                output.write(' vlan trunk allowed ' + str(d_vlan) + ',' + str(v_vlan) + '\n')
